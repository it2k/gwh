...
class Hook
{
    ...
    public function __construct($path = '', array $options = array())
    {
        ...
        $resolver->setDefaults(array(
            ...
            'loggerInstanceName'    => 'gwh',
            'loggerConfiguration'   => array(),
        ));
        $this->options = $resolver->resolve($options);

        \Logger::configure($this->options['loggerConfiguration']);
        $this->logger = \Logger::getLogger($this->options['loggerInstanceName']);
    ...
    private function checkPermissions($securityCode, $author, $host)
    {
        if ($securityCode && $securityCode != $this->request->getSecurityCode()) {
            $this->logger->warn('Security code not match');
        ...
        if (!$this->checkAllow($host, $this->request->getHost())) {
            $this->logger->warn('Host ' . $this->request->getHost() . ' not allowed on this branch');

            return false;
        }

        if (!$this->checkAllow($author, $this->request->getAuthor())) {
            $this->logger->warn('Author ' . $this->request->getAuthor() . ' not allowed on this branch');
    ...
    public function execute()
    {
        ...
        if ($repository = $this->getRepository($this->request->getRepository())) {
            ...
            if (!empty($this->commandsList) && $this->getRepositoryCount() == 0) {
                $this->logger->warn('Repository: ' . $this->request->getRepository() . ' and branch: ' . $this->request->getBranch() . ' not found in the settings');
            }
        }
